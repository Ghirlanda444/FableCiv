...
(function (AU) {
  ...
  node({ id: 'p_publichealth', pool: F, cat: 'Sustenance', name: 'Public Health', joke: 'Doctors Now Wash Their Hands (Mostly)', trigger: { type: 'state', cond: ['event', 'foodSurplus'], turns: 20, desc: 'Positive Food for 20 turns' }, unlocks: { building: 'hospital' } });
  node({ id: 'p_underground', pool: F, cat: 'Sustenance', name: 'Underground Pipes', joke: 'We Put The Poop Somewhere Else', trigger: { type: 'threshold', cond: ['pop', 14], desc: 'Reach 14 population in a settlement' }, unlocks: { building: 'sewer' } });
  ...
  node({ id: 'p_patentoffice', pool: F, cat: 'Craft', name: 'Patent Office', joke: 'Somebody Patented The Wheel, Again', trigger: { type: 'threshold', cond: ['unitsBuilt', 10], desc: 'Train 10 units' }, fx: { unitsStartXp: 5 } });
  node({ id: 'p_coalfire', pool: F, cat: 'Craft', name: 'Coal Furnace', joke: 'The Fire Never Goes Out Now, Ever', trigger: { type: 'state', cond: ['event', 'scienceSurplus'], turns: 15, desc: 'Positive Knowledge for 15 turns' }, fx: { yieldMult: { science: 1.15 } } });
  ...
  pair({ id: 'p_slum_grit', name: 'Slum Grit', joke: 'Nobody Is Happy But Everyone Works Hard', trigger: { type: 'threshold', cond: ['totalPop', 40], desc: 'Reach 40 total population' }, fx: { growthMult: 1.15 } },
       { id: 'p_garden_suburb', name: 'Garden Suburb', joke: 'A Tree, On Purpose, In The City', trigger: { type: 'state', cond: ['event', 'foodSurplus'], turns: 15, desc: 'Positive Food for 15 turns' }, fx: { happinessBonus: 3 } });
  ...
  node({ id: 'p_greenbelt', pool: B, name: 'Green Belt', joke: 'We Left Some Trees Alone On Purpose', trigger: { type: 'threshold', cond: ['building', 'monument', 4], desc: 'Build Monuments in 4 settlements' }, unlocks: { national: 'national_park' } });
