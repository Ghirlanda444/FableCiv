...
(function (AU) {
  ...
  pair({ id: 'x_bigcrowd', name: 'Big Crowd Energy', joke: 'Everyone Showed Up At Once', trigger: { type: 'threshold', cond: ['totalPop', 65], desc: 'Reach 65 total population' }, fx: { happinessBonus: 3 } },
       { id: 'x_quietblock', name: 'Quiet Neighborhood', joke: 'Nobody Here Wants Trouble', trigger: { type: 'state', cond: ['event', 'content'], turns: 25, desc: 'No unhappy settlement for 25 turns' }, fx: { growthMult: 1.15 } });
  pair({ id: 'x_globalchain', name: 'Global Chains', joke: 'The Same Sign On Every Corner', trigger: { type: 'threshold', cond: ['met', 4], desc: 'Know 4 other empires' }, fx: { goldPerSettlement: 2 } },
       { id: 'x_localshop', name: 'Local Shops', joke: 'We Kept It Small On Purpose', trigger: { type: 'threshold', cond: ['settlements', 8], desc: 'Found 8 settlements' }, fx: { capitalMult: { gold: 1.15 } } });
  pair({ id: 'x_armsrace', name: 'Arms Race', joke: 'Whoever Has More Toys Wins, Probably', trigger: { type: 'threshold', cond: ['military', 16], desc: 'Own 16 military units' }, fx: { combatBonus: 3 } },
       { id: 'x_peacedividend', name: 'Peace Dividend', joke: 'We Spent The War Money On Not War', trigger: { type: 'state', cond: ['event', 'peace'], turns: 25, desc: '25 turns without war' }, fx: { buildingCostMult: 0.85 } });
  pair({ id: 'x_eyesinsky', name: 'Eyes In The Sky', joke: 'We Can See Everything Now, Mostly', trigger: { type: 'threshold', cond: ['explored', 1500], desc: 'Explore 1500 tiles' }, fx: { reconSight: 2 } },
