...
(function (AU) {
  ...
  var F = 'foundation';
  node({ id: 'fire', pool: F, cat: 'Sustenance', name: 'Cookfire', joke: 'We Have Fire, Apparently', trigger: { type: 'threshold', cond: ['feature', 'forest', 6], desc: 'Own 6 Forest tiles' }, fx: { tileBonus: [{ when: 'forest', yields: { food: 1 } }] } });
  ...
  node({ id: 'cave', pool: F, cat: 'Shelter', name: 'Tidy Cave', joke: 'Cave, But Organized', trigger: { type: 'discovery', cond: ['adjacent', 'mountain', 1], turns: 10, desc: 'Hold a settlement next to a mountain for 10 turns' }, fx: { settlementSiteBonus: [{ when: 'mountain', yields: { production: 1 } }] } });
  ...
  node({ id: 'chief', pool: F, cat: 'Kinship', name: 'Big Chief', joke: 'Somebody Has To Be In Charge', fx: { commandBonus: 2 }, trigger: { type: 'threshold', cond: ['military', 4], desc: 'Own 4 military units' }, unlocks: { unit: 'commander' } });
  node({ id: 'rule', pool: F, cat: 'Kinship', name: 'First Rule', joke: 'We Made Up A Rule', trigger: { type: 'state', cond: ['event', 'kinfolkFriend'], turns: 20, desc: 'Stay on good terms with a Free city for 20 turns' }, fx: { culturePerSettlement: 1 } });
  node({ id: 'talking', pool: F, cat: 'Kinship', name: 'Talking Works', joke: 'Turns Out Talking Works Too', trigger: { type: 'discovery', cond: ['met', 2], turns: 10, desc: 'Know two other empires for 10 turns' }, fx: { attitudeBonus: 5 }, hub: 'looking_at_us' });
  ...
  node({ id: 'clay', pool: F, cat: 'Craft', name: 'Clay Thing', joke: 'We Figured Out Clay Does A Thing', trigger: { type: 'discovery', cond: ['riverTiles', 4], desc: 'Work 4 river tiles' }, fx: { buildingCostMult: 0.95 } }); // bricks: buildings 5% cheaper
  node({ id: 'wheels', pool: F, cat: 'Craft', name: 'Round Things', joke: 'Wheels Are Good, Actually', trigger: { type: 'threshold', cond: ['improvedTiles', 9], desc: 'Work 9 improved tiles' }, unlocks: { unit: 'caravan' }, hub: 'market_guy' });
  ...
  node({ id: 'otherguys', pool: F, cat: 'Wayfinding', name: 'Other Guys', joke: 'We Found The Other Guys', trigger: { type: 'discovery', cond: ['event', 'meetCS'], turns: 20, desc: 'Know a Free city for 20 turns' }, fx: { tiesPerTurn: 1 } });
  ...
  node({ id: 'map', pool: F, cat: 'Wayfinding', name: 'Bad Map', joke: 'Somebody Drew A Map (Badly)', trigger: { type: 'state', cond: ['exploredContinent', 40], turns: 1, desc: 'Explore 40% of your continent' }, fx: { revealContinent: true } });
  ...
  node({ id: 'tamed', pool: B, name: 'Tamed Thing', joke: 'Somebody Tamed A Thing', trigger: { type: 'discovery', cond: ['resource', 'horses|cattle', 1], desc: 'Own Horses or Cattle' }, fx: { cavalryBonus: 2 } });
  ...
  node({ id: 'game', pool: B, name: 'A Game', joke: 'The Kids Made Up A Game', trigger: { type: 'state', cond: ['event', 'peace'], turns: 10, desc: '10 turns without war' }, fx: { capitalYields: { culture: 1 }, happinessBonus: 1 } });
