...
(function (AU) {
  ...
  var F = 'foundation';
  node({ id: 'fire', pool: F, cat: 'Sustenance', name: 'Cookfire', joke: 'We Have Fire, Apparently', trigger: { type: 'threshold', cond: ['feature', 'forest', 3], desc: 'Work 3 Forest tiles' }, fx: { tileBonus: [{ when: 'forest', yields: { food: 1 } }] }, unlocks: { improvement: 'cookfire' } });
  ...
  node({ id: 'berry', pool: F, cat: 'Sustenance', name: 'Berry Luck', joke: 'Somebody Ate A Weird Berry And Lived', trigger: { type: 'discovery', cond: ['resource', 'bananas|wine|spices', 1], desc: 'Own a settlement next to an edible plant resource' }, fx: { capitalYields: { food: 1 } } });
  node({ id: 'storage', pool: F, cat: 'Sustenance', name: 'Rock Pantry', joke: 'Big Rock Storage Idea', trigger: { type: 'threshold', cond: ['pop', 3], desc: 'Reach 3 population in a settlement' }, unlocks: { building: 'granary' } });
  node({ id: 'irrigation', pool: F, cat: 'Sustenance', name: 'Dirt Watering', joke: 'Watering The Dirt On Purpose', trigger: { type: 'discovery', cond: ['river', 1], desc: 'Settle next to a river' }, fx: { tileBonus: [{ when: 'river', yields: { food: 1 } }] } });
  ...
  node({ id: 'fence', pool: F, cat: 'Shelter', name: 'First Fence', joke: 'Somebody Built a Fence', trigger: { type: 'state', cond: ['event', 'combat'], desc: 'Survive a fight' }, unlocks: { unit: 'settler' } });
  ...
  node({ id: 'dig', pool: F, cat: 'Shelter', name: 'Hill Hole', joke: 'We Dug A Hole On Purpose', trigger: { type: 'threshold', cond: ['tiles', 'hills', 3], desc: 'Own 3 Hill tiles' }, fx: { hillsDefense: 3 } });
  node({ id: 'palisade', pool: F, cat: 'Shelter', name: 'Stick Ring', joke: 'Sticks In A Circle Count As A Wall', trigger: { type: 'state', cond: ['event', 'undamaged'], turns: 8, desc: 'No settlement damaged for 8 turns' }, unlocks: { building: 'walls' } });
  node({ id: 'sleepspot', pool: F, cat: 'Shelter', name: 'Big Sleep Spot', joke: 'Everyone Sleeps In The Same Spot Now', trigger: { type: 'threshold', cond: ['pop', 4], desc: 'Reach 4 population in a settlement' }, fx: { housingBonus: 1 } });
  node({ id: 'cave', pool: F, cat: 'Shelter', name: 'Tidy Cave', joke: 'Cave, But Organized', trigger: { type: 'discovery', cond: ['adjacent', 'mountain', 1], desc: 'Settle next to a mountain' }, fx: { settlementSiteBonus: [{ when: 'mountain', yields: { production: 1 } }] } });
  node({ id: 'grouphug', pool: F, cat: 'Kinship', name: 'Group Hug', joke: 'Group Hug Diplomacy', trigger: { type: 'state', cond: ['event', 'content'], turns: 8, desc: 'No unhappy settlement for 8 turns' }, unlocks: { unit: 'migrant' } });
  node({ id: 'chief', pool: F, cat: 'Kinship', name: 'Big Chief', joke: 'Somebody Has To Be In Charge', trigger: { type: 'threshold', cond: ['military', 2], desc: 'Own 2 military units' }, unlocks: { unit: 'commander' } });
  node({ id: 'rule', pool: F, cat: 'Kinship', name: 'First Rule', joke: 'We Made Up A Rule', trigger: { type: 'state', cond: ['event', 'kinfolkFriend'], turns: 10, desc: 'Stay on good terms with a Free city for 10 turns' }, fx: { hubsEnabled: true } });
  node({ id: 'talking', pool: F, cat: 'Kinship', name: 'Talking Works', joke: 'Turns Out Talking Works Too', trigger: { type: 'discovery', cond: ['met', 1], desc: 'Meet another empire' }, fx: { diplomacyBasic: true }, hub: 'looking_at_us' });
  node({ id: 'elder', pool: F, cat: 'Kinship', name: 'The Elder', joke: 'The Old Lady Knows Things', trigger: { type: 'threshold', cond: ['pop', 5], desc: 'Reach 5 population in a settlement' }, fx: { greatSlot: 'elder' }, hub: 'old_lady' });
  node({ id: 'cookturn', pool: F, cat: 'Kinship', name: 'Cook Rota', joke: "Somebody's Turn To Cook", trigger: { type: 'state', cond: ['event', 'foodSurplus'], turns: 3, desc: 'Positive Food for 3 turns' }, fx: { happinessBonus: 1 } });
  node({ id: 'truce', pool: F, cat: 'Kinship', name: 'Mostly Peace', joke: 'We Agreed Not To Hit Each Other (Mostly)', trigger: { type: 'discovery', cond: ['event', 'peacefulContact'], desc: 'First contact without a fight' }, fx: { influencePerTurn: 1 } });
  node({ id: 'shiny', pool: F, cat: 'Craft', name: 'Shiny Rocks', joke: 'We Found Shiny Rocks', trigger: { type: 'threshold', cond: ['tiles', 'hills', 2], desc: 'Work 2 stone or hill tiles' }, unlocks: { unit: 'slinger' } });
  node({ id: 'point', pool: F, cat: 'Craft', name: 'Pointy Sticks', joke: 'Sticks Now Have a Point', trigger: { type: 'state', cond: ['settlements', 1], turns: 5, desc: 'Hold a settlement for 5 turns' }, unlocks: { unit: 'warrior' }, cheap: true });
  ...
  node({ id: 'sharper', pool: F, cat: 'Craft', name: 'Sharper Rocks', joke: 'Rocks That Are Sharper Than Other Rocks', trigger: { type: 'threshold', cond: ['improvedTiles', 5], desc: 'Work 5 improved tiles' }, fx: { capitalYields: { production: 2 } } });
  node({ id: 'clay', pool: F, cat: 'Craft', name: 'Clay Thing', joke: 'We Figured Out Clay Does A Thing', trigger: { type: 'discovery', cond: ['river', 1], desc: 'Settle next to a river or lake' }, unlocks: { improvement: 'kiln' } });
  node({ id: 'wheels', pool: F, cat: 'Craft', name: 'Round Things', joke: 'Wheels Are Good, Actually', trigger: { type: 'threshold', cond: ['event', 'roadLink'], desc: 'Connect two settlements by road' }, unlocks: { unit: 'caravan' }, hub: 'market_guy' });
  ...
  node({ id: 'farwalk', pool: F, cat: 'Wayfinding', name: 'Long Walk', joke: 'We Walked Really Far', trigger: { type: 'threshold', cond: ['explored', 60], desc: 'Explore 60 tiles' }, fx: { reconSight: 1, reconMoves: 1 } });
  node({ id: 'overhill', pool: F, cat: 'Wayfinding', name: 'Over That Hill', joke: "There's Stuff Over That Hill", trigger: { type: 'discovery', cond: ['event', 'natural'], desc: 'Find a natural wonder' }, fx: { capitalYields: { culture: 2 } } });
  node({ id: 'otherguys', pool: F, cat: 'Wayfinding', name: 'Other Guys', joke: 'We Found The Other Guys', trigger: { type: 'discovery', cond: ['event', 'meetCS'], desc: 'Meet a Free city' }, fx: { kinfolkBasic: true } });
  node({ id: 'path', pool: F, cat: 'Wayfinding', name: 'Path Marks', joke: "Marking The Path So We Don't Get Lost", trigger: { type: 'threshold', cond: ['roads', 3], desc: 'Own 3 road tiles' }, fx: { roadMoves: 1 } });
  node({ id: 'map', pool: F, cat: 'Wayfinding', name: 'Bad Map', joke: 'Somebody Drew A Map (Badly)', trigger: { type: 'state', cond: ['exploredContinent', 40], desc: 'Explore 40% of your continent' }, fx: { revealContinent: true } });
  ...
  node({ id: 'kids', pool: B, name: 'Kid Training', joke: 'The Kids Are Getting Good At This', trigger: { type: 'threshold', cond: ['unitsBuilt', 3], desc: 'Train 3 units' }, fx: { unitsStartXp: 5 } });
  node({ id: 'rockpile', pool: B, name: 'Bigger Rock Pile', joke: 'We Made A Bigger Rock Pile', trigger: { type: 'threshold', cond: ['pop', 6], desc: 'Reach 6 population in a settlement' }, fx: { housingBonus: 1 } });
  ...
  node({ id: 'storm', pool: B, name: 'Storm Yell', joke: 'We Yelled At A Storm And It Stopped (Coincidence)', trigger: { type: 'discovery', cond: ['event', 'disaster'], desc: 'Survive a disaster' }, fx: { happinessBonus: 1 }, note: 'disasters do not exist yet' });
