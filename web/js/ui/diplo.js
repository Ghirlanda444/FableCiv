...
(function (AU) {
  ...
  UI.render = function () {
    ...
    if (civ.minor) {
      ...
      info = '<div><b>' + esc(d.ability.name) + ':</b> ' + esc(G.abilityDesc(d.ability)) + ' <span class="stat">(' + _('for the Patron)') + '</span></div><div>' + _('Ties') + ': <b>' + mine + '</b>/100 · ' + CS.tierName(mine) + (pat === p.idx ? ' · ' + _('you are the Patron') : '') + '</div>' +
