...
(function (AU) {
  ...
  A.play = function (name) {
    ...
    el.play().then(function () { A.fadeTo(el, A.volume); if (old) A.fadeTo(old, 0, function () { old.pause(); old.src = ''; }); }).catch(function () { A.cur = old; A.curName = oldName; });
  };
  ...
  A.setEnabled = function (on) { A.enabled = !!on; if (!on) { if (A.cur) { A.cur.pause(); A.cur.src = ''; } A.cur = null; A.curName = null; } else if (A.want) A.play(A.want); };
