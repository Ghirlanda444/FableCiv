...
(function (AU) {
  ...
  T.LIST = [
    ...
    { id: 'v2insight', title: _('An Insight'), text: _('An Insight 🔮 is a one-time decision that fires on your first deeds. Choose once: the branch becomes a permanent trait of your empire, and its twin is gone for good.'), when: function (g, p) { return !!g.v2 && !!(p.v2 && p.v2.pendingHubs.length); } },
    { id: 'v2pair', title: _('Paired Sparks'), text: _('Branch Sparks come in pairs. The first of a pair to fire locks the other for the whole game, so steer your empire towards the twin you want.'), when: function (g, p) { return !!g.v2 && !!(p.v2 && Object.keys(p.v2.locked).length); } },
    { id: 'spark', title: _('Your first Spark'), text: _('A Spark 💡 is a small deed tied to a technology (work a pasture, meet an empire…). Do it before the research is finished and the technology also grants its Mastery ⭐, a permanent bonus. Civics work the same with Insights.'), when: function (g, p) { return !g.v2 && (!!(p.boosts && Object.keys(p.boosts).length)); } },
    ...
    { id: 'policy', title: _('Policy cards'), text: _('Civics give you policy cards and your government gives you slots. Open ☰ → Government') + ' &amp; policies, tap a card to slot it, and change cards whenever you like, for free.', when: function (g, p) { return !g.v2 && (G.availablePolicies(p).length > 0); } },
    ...
    { id: 'era', title: _('A new era'), text: _('Eras come from your technologies and civics. Later eras cost more Knowledge and Heritage per step, unlock stronger units, and open new victories: Renown from the Modern era, Devotion from the Industrial era.'), when: function (g, p) { return !g.v2 && ((p.era || 0) >= 1); } }
  ];
