    ...
    if (CS) {
      ...
      minors.forEach(function (m) {
        ...
        html += '<div class="row"><div class="swatch" style="width:14px;height:40px;border-radius:4px;background:' + G.civColor(m) + '"></div><div class="grow"><b>' + T.icon + ' ' + d.name + '</b> <span class="pill">' + T.name + '</span> ' + (!m.alive ? '<span class="pill war">gone</span>' : war ? '<span class="pill war">' + _('At war') + '</span>' : '<span class="pill' + (tier >= 3 && pat === p.idx ? ' peace' : '') + '">' + CS.tierName(mine) + (pat === p.idx ? ' · ' + _('Patron') : '') + '</span>') +
          ...
          '<small><span class="strong">' + d.ability.name + ':</span> ' + d.ability.desc + (g.v2 && /technolog|civic|Oligarchy|Monarchy|Theocracy|Autocracy/i.test(d.ability.desc) ? ' <i>(' + _('Divergence: technology = Spark, civic = Insight, no governments') + ')</i>' : '') + '</small>' + (g.v2 && AU.Society && m.alive && !war ? (function () { var PER = AU.Society.PERSUASION[AU.Society.persuasionOf(m)], bs = AU.Society.bondState(g, p, m); return '<small>' + PER.icon + ' ' + _('Listens to') + ' <span class="strong">' + _(PER.name) + '</span>: ' + _(PER.desc) + ' · 🤝 ' + _('Bond') + ': ' + (bs.bonded ? '<span class="strong">' + _('sealed') + '</span> (' + AU.Society.bondUpkeep(g, p) + ' 🎯/' + _('turn') + ')' : bs.ok ? '<span class="strong">' + _('possible now') + '</span> (' + bs.upkeep + ' 🎯/' + _('turn') + ')' : bs.why) + '</small>'; })() : '') + (!g.v2 && m.alive && !war && tier >= 4 && pat === p.idx ? '<small>🤝 ' + _('Union') + ': ' + (us.ok ? '<span class="strong">possible now</span>' : us.why) + '</small>' : '') + '</div>' +
