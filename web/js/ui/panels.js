...
(function (AU) {
  ...
  P.render_city = function (app, g, data) {
    ...
    if (g.v2 && AU.Synthesis && s.isCity && (civ.era || 0) >= AU.Synthesis.MIN_ERA) {
      ...
      if (!SY.hasLab(s)) html += '<p class="stat">' + _('A Research Lab here would let this City turn Knowledge into a lasting supply of any strategic resource.') + '</p>';
        ...
        html += '<p class="stat">' + _('Turn Knowledge into a lasting supply of a strategic resource') + ': ' + syCost + ' 📚 ' + _('for') + ' +' + SY.SUPPLY + ' ' + _('supply') + ' (' + _('you have') + ' ' + syStudy + ' 📚 · ' + _('every synthesis makes the next dearer') + ')</p>';
  ...
  P.render_web = function (app, g, data) {
    ...
    html += '<p class="stat">' + _('Sparks come from what you do: work tiles, explore, fight, meet people. Any Spark you cannot wait for can be studied with Knowledge (cheap ones for less, branch ones for more). Paired choices lock their twin for the whole game.') + '</p>';
    ...
    function nodeRow(n) {
      var status = MW.status(p, n.id), can = status === 'locked_unmet' && MW.canStudy(g, p, n.id), cls = status === 'unlocked' ? 'done' : status === 'locked_permanent' ? 'locked' : (can ? 'active clickable' : ''), badge = status === 'unlocked' ? '✅' : status === 'locked_permanent' ? '🔒' : can ? '📚' : '💡';
      ...
      return '<div class="row ' + cls + '" data-action="study" data-id="' + n.id + '"><span class="hall-medal">' + badge + '</span><div class="grow"><b>' + n.name + (n.pair ? ' <span class="kind-pill wildcard">' + _('paired') + '</span>' : '') + '</b><small><i>“' + n.joke + '”</i></small><small>' + (status === 'locked_permanent' ? _('Locked for good: you chose') + ' ' + (AU.V2.NODE_BY_ID[n.pair] ? AU.V2.NODE_BY_ID[n.pair].name : '') : (status === 'unlocked' ? _('Sparked on turn') + ' ' + st.unlocked[n.id] : n.trigger.desc + streak + ' · ' + _('or study for') + ' ' + MW.studyCost(g, p, n) + ' 📚')) + '</small>' + (unl.length ? '<small>' + _('Unlocks') + ': ' + unl.join(', ') + '</small>' : '') + (n.fx ? '<small>' + describeFx(n.fx) + '</small>' : '') + '</div></div>';
    }
    if (tab === 'foundation') {
      var cats = []; nodes.forEach(function (n) { if (n.pool === 'foundation' && cats.indexOf(n.cat) < 0) cats.push(n.cat); });
      cats.forEach(function (c) { html += '<div class="section"><h3>' + (CAT_ICON[c] || '') + ' ' + _(c) + '</h3>'; nodes.filter(function (n) { return n.pool === 'foundation' && n.cat === c; }).forEach(function (n) { html += nodeRow(n); }); html += '</div>'; });
      ...
      var rc = MW.reformCost(g, p), used = st.reformUsed && st.reformUsed[st.era];
      html += '<p class="stat">🎭 ' + Math.floor(st.heritage || 0) + ' ' + _('Heritage') + ' (+' + y.culture.toFixed(1) + ') · ' + _('Reform') + ': ' + (used ? _('already used this age') : rc + ' ' + _('Heritage, once per age: take back one Insight and choose its other branch')) + '</p>';
      var any = false; AU.V2.HUBS.forEach(function (h) { var b = st.traits[h.id]; if (!b) return; any = true; var br = h.branches.filter(function (x) { return x.id === b; })[0]; var canR = MW.canReform(g, p, h.id); html += '<div class="row done"><span class="hall-medal">🔮</span><div class="grow"><b>' + h.name + '</b><small>' + (br ? br.name + ' — ' + describeFx(br.fx) : '') + '</small>' + (canR ? h.branches.filter(function (x) { return x.id !== b; }).map(function (x) { return '<button class="small" data-action="reform" data-hub="' + h.id + '" data-branch="' + x.id + '">🎭 ' + _('Reform to') + ' ' + x.name + ' (' + rc + ')</button>'; }).join(' ') : '') + '</div></div>'; });
  ...
  P.action = function (app, name, d) {
    ...
    switch (name) {
      ...
      case 'study': { var sn = AU.V2.NODE_BY_ID[d.id]; if (!sn || !AU.MasteryWeb.canStudy(g, p, d.id)) break; var doStudy = function () { if (AU.MasteryWeb.study(g, p, d.id)) { app.toast('💡 ' + _('Spark!') + ' ' + sn.name); app.refreshPanel(); app.refreshHud(); } }; if (sn.pair && AU.V2.NODE_BY_ID[sn.pair]) app.confirm(_('Study') + ' ' + sn.name + ' ' + _('for') + ' ' + AU.MasteryWeb.studyCost(g, p, sn) + ' 📚? ' + _('This locks') + ' ' + AU.V2.NODE_BY_ID[sn.pair].name + ' ' + _('for the whole game.'), doStudy); else doStudy(); break; }
      ...
      case 'synth': { s = g.settlements[+d.id]; var SYr = AU.RESOURCES[d.res]; if (!s || !SYr || !AU.Synthesis.canRun(g, p, s) || !AU.Synthesis.canAfford(g, p)) break; app.confirm(_('Synthesise') + ' ' + SYr.icon + ' ' + SYr.name + ' ' + _('for') + ' ' + AU.Synthesis.cost(g, p) + ' 📚? ' + _('Supply') + ' +' + AU.Synthesis.SUPPLY + ', ' + _('for the whole game.'), function () { if (AU.Synthesis.run(g, p, d.res)) { app.toast('🧪 ' + SYr.name + ' ' + _('synthesised.')); app.refreshPanel(); app.refreshHud(); } }); break; }
