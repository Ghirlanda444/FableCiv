...
(function (AU) {
  ...
  P.render_empire = function (app, g) {
    ...
    html += '<div class="section"><h3>' + G.leaderName(p) + ' of ' + d.name + '</h3><div class="yields">' + yieldsHtml(y, { skip: ['food', 'happiness'], plus: true }) + '<span>💰 ' + Math.floor(p.gold) + ' treasury</span>' + (g.v2 ? '<span>🎯 ' + Math.floor(p.influence || 0) + ' ' + _('Influence') + ' (+' + G.influenceIncome(g, p) + ') · ' + _('pays for tile claims beyond the first three') + '</span>' + (AU.Smoke ? '<span>🏭 ' + AU.Smoke.total(g, p) + ' ' + _('Smoke') + ' · ' + _('world') + ' ' + AU.Smoke.world(g) + '</span>' : '') + (AU.Climate ? (function () { var cs = AU.Climate.state(g); return '<span>🌡️ ' + _('Climate') + ': ' + AU.Climate.describe(cs.warmth) + (cs.pending ? ' · ' + _('shift in') + ' ' + (cs.pending.at - g.turn) + ' ' + _('turns') + ' (' + cs.pending.tiles.length + ' ' + _('tiles') + ')' : '') + '</span>'; })() : '') : '') + '<span>unit upkeep ' + y.upkeep + '</span></div><p class="stat">' + d.ability.name + ': ' + G.abilityDesc(d.ability) + '</p></div>';
