...
(function (AU) {
  ...
  GP.options = function (g, u) {
    ...
    switch (type) {
      ...
      case 'engineer': out.push({ action: 'greatuse', label: '⚙️ ' + (own && s.isCity ? 'Add ' + GP.burst(g, civ, 150) + ' Production to ' + (s.queue.length ? AU.Panels ? AU.Panels.itemName(g, civ, s.queue[0]) : 'the build' : 'the next build') : 'Turn skill into ' + GP.burst(g, civ, 150) + ' Gold'), ok: !!own && (!s.isCity || s.queue.length > 0), why: !own ? 'Move into one of your settlements.' : 'Queue something in this City first.' }); break;
