  ...
  GP.use = function (g, u) {
    ...
    switch (type) {
      ...
      case 'scientist': { if (g.v2 && AU.MasteryWeb) { if (!AU.MasteryWeb.grantFreeSpark(g, civ)) return false; msg = u.name + ' ' + _('fires a Spark!'); break; }
        var tech = (civ.currentTech ? AU.TECH_BY_ID[civ.currentTech] : null) || G.availableTechs(civ).sort(function (a, b) { return a.cost - b.cost; })[0]; if (!tech) return false; G.learnTech(g, civ, tech.id); msg = u.name + ' discovers ' + tech.name + '!'; break; }
