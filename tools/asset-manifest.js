...
for (const [id, r] of Object.entries(AU.RESOURCES)) items.push({ kind: 'resources', id, name: r.name, size: '512x512', prompt: `Cute cartoon game icon of ${r.name} as a map resource: a single chunky, clearly recognisable ${r.name} object with a thick outline, bright colours, large and centred, isolated on a plain white background. ${STYLE}` });
for (const c of AU.CIVS) items.push({ kind: 'civs', id: c.id, name: c.name + ' emblem', size: '512x512', prompt: `Round cartoon shield emblem symbolising the ${c.name} civilization: one bold symbolic motif (an animal, plant, weapon, building or celestial symbol associated with ${c.name}), no people, no faces, no text, gold and ${c.color} colours, thick outline, plain white background. ${STYLE}` });
const TERRAIN_DESC = {
  ...
  marsh: 'a cartoon swamp seen from directly above, dark water pools, reeds, cattails and lily pads', hills: 'grassy green rounded cartoon hill mounds with soft shading and small rocks, many mounds side by side'
};
