...
const UNIT_DESC = Object.assign({}, GREAT_DESC, { caravan: 'a merchant leading a camel loaded with colourful bundles, carpets and a big sack of goods',
...
for (const [id, u] of Object.entries(AU.UNITS)) items.push({ kind: 'units', id, name: u.name, size: '512x512', prompt: `Chibi 3D game character, exactly one subject alone in the picture: ${UNIT_DESC[id] || u.name}, ${ROLE[u.cls] || 'the role clearly visible'}, big head and small stocky body, full figure visible from head to toe, standing on the ground, 3/4 view facing left, small in the frame with empty space around it. ${STYLE}` });
