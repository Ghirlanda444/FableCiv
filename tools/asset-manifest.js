...
const TERRAIN_DESC = {
  ...
  forest: 'a dense cartoon forest canopy seen from directly above, many round chunky pine and oak tree tops packed tightly together, no ground visible', jungle: 'a dense cartoon jungle canopy seen from directly above, huge glossy leaves, palms and vines packed tightly together, no ground visible',
  marsh: 'a cartoon swamp seen from directly above, dark water pools, reeds, cattails and lily pads', hills: 'rolling grassy cartoon hills seen from directly above, rounded mounds with soft shadows and small rocks'
};
for (const [id, d] of Object.entries(TERRAIN_DESC)) items.push({ kind: 'terrain', id, name: (AU.TERRAIN[id] ? AU.TERRAIN[id].name : AU.FEATURES[id] ? AU.FEATURES[id].name : 'Hills') + ' texture', size: '512x512', nobg: true, prompt: `Seamless tileable top-down texture of ${d}, perfectly flat, evenly lit, no horizon, no border. ${SCENE_STYLE}` });
const FEATURE_DESC = {
