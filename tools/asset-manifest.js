...
const UNIT_DESC = Object.assign({}, GREAT_DESC, { caravan: 'a two-humped camel as the main subject, loaded with colourful bundles, rolled carpets and sacks of goods, a small merchant walking beside it holding its rope',
  apostle: 'a travelling holy man in a colourful red and gold robe holding up a big golden holy book, with a tall staff topped by a shining sun symbol', inquisitor: 'a stern cleric in a dark crimson hooded robe holding a burning torch and a rolled scroll, a silver chain and medallion on the chest',
...
// The job of a unit must be readable at a glance: the weapon or tool is large, held up and fully visible.
const ROLE = { melee: 'the weapon and shield held up high, large and fully visible', ranged: 'aiming the weapon at the viewer\'s left with an arrow or shot visible, the bow or gun large and fully visible', antcav: 'the very long spear or pike held diagonally across the whole picture, clearly longer than the figure', cavalry: 'riding a horse in mid-gallop, weapon raised', siege: 'the siege engine or gun fills most of the frame, the crew tiny beside it', recon: 'shading the eyes with one hand and looking far away, a spyglass or map in the other hand', naval: 'the ship fills the frame', navalRanged: 'the ship fills the frame', air: 'the aircraft fills the frame', civilian: 'tools and bundles clearly visible', religious: 'holding a glowing holy symbol high' };
for (const [id, u] of Object.entries(AU.UNITS)) items.push({ kind: 'units', id, name: u.name, size: '512x512', prompt: `Chibi 3D game character, exactly one subject alone in the picture: ${UNIT_DESC[id] || u.name}, ${isVehicle(id) ? 'the whole vehicle large and fully visible, no rider on a horse' : (ROLE[u.cls] || 'the role clearly visible')}, big head and small stocky body, full figure visible from head to toe, standing on the ground, 3/4 view facing left, small in the frame with empty space around it. ${STYLE}` });
for (const c of AU.CIVS) if (c.uu) items.push({ kind: 'units', id: c.uu.id, name: c.uu.name + ' (' + c.name + ' unique unit)', size: '512x512', prompt: `Chibi 3D game character: ${UU_LOOKS[c.uu.id] || c.uu.name}, big head and small stocky body, full figure visible from head to toe, standing on the ground, 3/4 view facing left, small in the frame with empty space around it. ${STYLE}` });
...
for (const c of AU.CIVS) for (const l of c.leaders) items.push({ kind: 'leaders', id: l.id, name: `${l.name} (${c.name})`, size: '512x640', prompt: `Chibi 3D character portrait bust of the historical ${l.name}, ${l.title} of ${c.name}, with a recognisable likeness: ${LEADER_LOOKS[l.id] || 'period-accurate clothing and headgear'}. Exactly one person, head and shoulders, facing slightly left, big head with large expressive eyes, confident expression, smooth glossy toy-like shading, same framing as a collectible card portrait, centred, isolated on a plain white background, no text, no frame. ${STYLE}` });
const RESOURCE_DESC = {
  ...
  copper: 'a pile of shiny orange copper ore nuggets with a small copper ingot', rubber: 'a rubber tree tapping cup with white latex and a stack of black rubber tyres', aluminum: 'a stack of silvery aluminum ingots with a shiny metal sheet', uranium: 'a glowing green uranium ore chunk in a small lead-grey cask with a radiation symbol'
};
...
for (const c of AU.CIVS) items.push({ kind: 'civs', id: c.id, name: c.name + ' emblem', size: '512x512', prompt: `Round game emblem badge of the ${c.name} empire: one bold, instantly recognisable symbol filling the centre: ${CIV_SYMBOLS[c.id] || 'a bold symbol associated with ' + c.name}. Flat bright colours, ${c.color} background, thick gold rim, thick clean outlines, symmetrical and centred, chibi cartoon vector sticker style, no text, no letters, no extra people, isolated on a plain white background. ${SCENE_STYLE}` });
...
items.push({ kind: 'keyart', id: 'title_landscape', name: 'Title key art (landscape)', size: '1536x864', nobg: true, hires: true, prompt: `Wide cinematic key art for a cute chibi empire-building strategy game, ${KEYART_SCENE}, the characters fill the lower two thirds and the sky stays open at the top. Absolutely no text, no letters, no logo, no user interface, no watermark, no border. ${SCENE_STYLE}` });
items.push({ kind: 'keyart', id: 'title_portrait', name: 'Title key art (portrait, phone)', size: '864x1536', nobg: true, hires: true, prompt: `Tall vertical phone-screen key art for a cute chibi empire-building strategy game, ${KEYART_SCENE}, the characters fill the lower half and the castle and sky fill the upper half. Absolutely no text, no letters, no logo, no user interface, no watermark, no border. ${SCENE_STYLE}` });
items.push({ kind: 'logo', id: 'tiny_empires', name: 'Logo', size: '1024x512', prompt: `Mobile game title logo: the two words "TINY EMPIRES" written in big chunky rounded 3D golden cartoon letters with a thick dark brown outline, glossy bevelled toy-like lettering, slightly arched, a tiny golden crown sitting on the first letter and two small green laurel leaves at the sides, in the style of Clash of Clans and Kingshot game logos. The words must be spelled exactly T-I-N-Y and E-M-P-I-R-E-S with no other words, no tagline, no subtitle. Centred, isolated on a plain white background, no watermark.` });
const outDir = path.join(__dirname, '..', 'web', 'assets');
fs.mkdirSync(outDir, { recursive: true });
fs.writeFileSync(path.join(outDir, 'manifest.json'), JSON.stringify(items.map(i => ({ kind: i.kind, id: i.id, file: `${i.kind}/${i.id}.${['terrain', 'techs', 'civics', 'thrones', 'keyart'].indexOf(i.kind) >= 0 ? 'jpg' : 'png'}`, name: i.name, size: i.size, nobg: !!i.nobg, hires: !!i.hires })), null, 1));
