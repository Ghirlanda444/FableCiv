...
| City growth (1 tile per pop, Influence-gated, Expansion building) | 7-tile founding, autoExpand, work-tile assignment | game.js foundSettlement/claimTile/autoExpand, citymap.js, AI expansion | To do (new logic) |
| City Focus slots (3, pop-gated) | 5 Town specializations + City upgrade | game.js specialization, panels | To do |
| Great People from hub payoffs | 7 Great types with points | core/greatpeople.js (113 lines) | To do (spawnNamed hook stubbed) |
| Warbands (3 units per tile, one exchange) | 1 unit per tile, per-unit combat, ZoC | core/units.js movement/attack/ZoC, renderer unit drawing, AI | To do |
...
3. **City growth** — center tile only, +1 tile per pop growth, tiles 4+ gated by Influence (new per-civ currency from Kinship nodes and traits) and an Expansion building (2–3 tiers), pending claim queue; City Focus slots; citymap UI.
4. **Warbands** — up to 3 units per tile as one stack, combined strength, casualties weakest-first, ZoC per stack; Commander unit; renderer cluster icon; AI.
